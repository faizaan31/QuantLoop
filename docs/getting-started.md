# Getting Started with QuantLoop

This guide walks you through installing QuantLoop, writing your first strategy, running a backtest, and interpreting the results.

## Installation

Requires Python 3.10+. QuantLoop is installed from source — clone the repository and use an editable install:
...
pip install -e ".[talib]"       # TA-Lib integration
```
...
Every QuantLoop strategy has two methods:
...
QuantLoop accepts Polars DataFrames. The minimum required column is `close`. For full OHLCV support, include `open`, `high`, `low`, `close`, and `volume`.
...
```python
from quantloop.plotting import plot_backtest, plot_returns_distribution
...
```python
from quantloop import optimize
...
For portfolio allocation strategies (momentum rotation, factor models, equal-weight baskets), QuantLoop provides a declarative `backtest_weights()` function. Instead of writing `preprocess()` + `next()`, you supply a long-format DataFrame with target weights per (date, symbol):
