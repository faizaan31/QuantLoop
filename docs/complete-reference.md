# QuantLoop Agent Reference

Complete reference for QuantLoop — a hybrid backtesting engine combining vectorized preprocessing (Polars) with event-driven execution for trading strategies.

## Architecture

QuantLoop uses a two-phase architecture:
...
Example:
```python
from quantloop import WeightStrategy
...
All indicators are Polars expressions used in `preprocess()`. Import as `from quantloop import indicators as ind`.
...
Import: `from quantloop.sizers import ...`
...
Import: `from quantloop.commissions import ...`
...
Import: `from quantloop import backtest, backtest_batch, optimize, optimize_multi, optimize_bayesian, walk_forward_analysis`
...
Import: `from quantloop import monte_carlo, detect_look_ahead_bias, permutation_test`
...
Import: `from quantloop.data import ...`

### Validation

```python
from quantloop.data import validate, ValidationResult
...
```python
from quantloop.data import fill_gaps, adjust_splits, drop_zero_volume, clip_outliers
...
```python
from quantloop.data import resample_ohlcv
...
Import: `from quantloop.metrics import ...`
...
Requires the plotting extra: `pip install -e ".[plotting]"`. Import: `from quantloop.plotting import ...`
...
```python
from quantloop.integrations.talib import talib_expr, talib_multi_expr, talib_series, TALibIndicators
...
Import: `from quantloop import backtest_weights, WeightBacktestResult`
...
```python
from quantloop import compute_next_actions
